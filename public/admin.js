...
function displayRequests() {
    ...
    container.innerHTML = filteredRequests.map(req => `
        <div class="glass-card rounded-lg border-l-4 ${statusColors[req.status]} ${newRequestIds.has(req.id) ? 'new-request-blink' : ''}" data-request-id="${req.id}">
            <div class="p-3">
        ...
        const recentHtml = recentRequests.map(req => `
            <div class="flex justify-between items-center p-3 bg-slate-700 rounded-lg">
                ...
                <span class="px-2 py-1 text-xs rounded-full ${
                    req.status === 'arandı' ? 'bg-emerald-500/20 text-emerald-300' :
